(function() {
  var Configuration, Consts, Logger, Module, compact, df, exports, extend, first, glob, globals, include, isArray, isBool, isEmpty, isNum, isString, last, m, merge, path, prependTime, put, vow, _, __label, _base, _base1, _base2, _debug, _error, _i, _info, _len, _log, _ref, _succ, _warn,
    __slice = [].slice,
  ...
  exports.Modules.Promisable = {
    included: function() {
      this.prototype._promise = vow.defer().promise();
      return this.prototype._status = Consts.PROMISE_STATUS.PENDING;
    },
    isPromise: function(promise) {
      if (promise == null) {
        promise = this._promise;
      }
      return vow.isPromise(promise);
    },
    isFulfilled: function(promise) {
      if (promise == null) {
        promise = this._promise;
      }
      return vow.isFulfilled(promise);
    },
    isRejected: function(promise) {
      if (promise == null) {
        promise = this._promise;
      }
      return vow.isRejected(promise);
    },
    isResolved: function(promise) {
      if (promise == null) {
        promise = this._promise;
      }
      return vow.isResolved(promise);
    },
    then: function(onFulfilled, onRejected, onNotify) {
      this._adoptPromise(this._context);
      return vow.when(this._promise, onFulfilled, onRejected, onNotify);
    },
    "catch": function(onRejected) {
      return this.then(null, onRejected);
    },
    fail: function(onRejected) {
      return vow.fail(this._promise, onRejected);
    },
    always: function(onResolve) {
      return vow.always(this._promise, onResolve);
    },
    progress: function(onProgress) {
      return vow.progress(this._promise, onProgress);
    },
    done: function(onFulfilled, onRejected, onNotify) {
      return vow.done(this._promise, onFulfilled, onRejected, onNotify);
    },
    delay: function(delay) {
      return vow.delay(this._promise, delay);
    },
    _fulfill: function(val) {
      return vow.fulfill(val);
    },
    ...
    _valueOf: function() {
      if ((this._promise != null) && this.isPromise()) {
        return this._promise.valueOf();
      }
    },
    _wrapPromise: function(promise) {
      this._adoptPromise(promise, true);
      this._setContext(promise);
      return this;
    },
    _adoptPromise: function(promise, isNotAPlusCompliant) {
      if (isNotAPlusCompliant == null) {
        isNotAPlusCompliant = false;
      }
      if (isNotAPlusCompliant && (promise.then != null) && (promise.thenCatch != null)) {
        promise.then((function(_this) {
          ...
          return function(err) {
            return _this._promise = _this._reject(err);
          };
        })(this));
      } else {
        if (this.isPromise(promise) && this.isResolved(promise)) {
          this._promise = vow.cast(promise);
        }
      }
